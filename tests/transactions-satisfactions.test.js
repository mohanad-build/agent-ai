...
// the same order as the names given. Each participant's `name` is set to the
// given string purely for test readability in failure output; the writers
// under test key and authorize by id, never by name.
...
describe('markPersonSatisfied', () => {
  it('a first satisfaction writes a person_satisfied event and a clientSatisfactions entry of only { at, actor }', () => {
    ...
    const result = markPersonSatisfied(AGENT_ID, created.transactionId, janeId, 'reco_information_guide', {
    ...
    expect(satisfiedEvents[0].payload).toEqual({ personId: janeId, itemId: 'reco_information_guide' });
    expect(result.facts.clientSatisfactions).toEqual({
      [janeId]: { reco_information_guide: { at: AT2, actor: 'agent' } },
    });
  });

  it('throws when personId is not a participant on the transaction, and writes nothing to disk', () => {
    const created = create();
    represent(created.transactionId, ['Jane Smith']);
    const before = readTransaction(AGENT_ID, created.transactionId, { baseDir });

    expect(() => markPersonSatisfied(AGENT_ID, created.transactionId, 'per-ffffffff', 'reco_information_guide', {
      at: AT2, actor: 'agent', baseDir, now: LATER,
    })).toThrow(/^markPersonSatisfied: 'per-ffffffff' is not a participant on transaction/);

    const onDisk = readTransaction(AGENT_ID, created.transactionId, { baseDir });
    expect(onDisk).toEqual(before);
  });

  it('throws when there are no participants on the transaction at all, and writes nothing to disk', () => {
    const created = create();

    expect(() => markPersonSatisfied(AGENT_ID, created.transactionId, 'per-ffffffff', 'reco_information_guide', {
      at: AT2, actor: 'agent', baseDir, now: LATER,
    })).toThrow(/^markPersonSatisfied: no participants on transaction/);
  ...
  it('throws when the id is a participant but holds no qualifying role', () => {
    ...
    expect(() => markPersonSatisfied(AGENT_ID, created.transactionId, lawyerId, 'reco_information_guide', {
      at: AT2, actor: 'agent', baseDir, now: LATER,
    })).toThrow(new RegExp(`^markPersonSatisfied: '${lawyerId}' is a participant on transaction .* but holds no role in`));
  });

  it('looks up the participant id by exact match: a differently-cased id is not the same participant', () => {
    const created = create();
    const [janeId] = represent(created.transactionId, ['Jane Smith']);
    const differentCase = janeId.toUpperCase();

    expect(() => markPersonSatisfied(AGENT_ID, created.transactionId, differentCase, 'reco_information_guide', {
      at: AT2, actor: 'agent', baseDir, now: LATER,
    })).toThrow(new RegExp(`^markPersonSatisfied: '${differentCase}' is not a participant on transaction`));
  });
  ...
  it('throws when the item is not client-scoped, naming the item', () => {
    ...
    expect(() => markPersonSatisfied(AGENT_ID, created.transactionId, janeId, 'deal_sheet', {
      at: AT2, actor: 'agent', baseDir, now: LATER,
  ...
  it("throws when the item is clientScope 'dated', naming the item", () => {
    ...
    expect(() => markPersonSatisfied(AGENT_ID, created.transactionId, janeId, 'buyer_representation_agreement', {
      at: AT2, actor: 'agent', baseDir, now: LATER,
  ...
  it('throws on an unknown item id for the type', () => {
    ...
    expect(() => markPersonSatisfied(AGENT_ID, created.transactionId, janeId, 'not_a_real_item', {
      at: AT2, actor: 'agent', baseDir, now: LATER,
  ...
  it('throws when the transaction does not exist', () => {
    expect(() => markPersonSatisfied(AGENT_ID, 'txn-20260715-00000000', 'per-00000000', 'reco_information_guide', {
      at: AT2, actor: 'agent', baseDir, now: LATER,
  ...
  it('satisfying one of two persons leaves the other in outstandingPersons (proven through resolveTransactionChecklist)', () => {
    const created = create();
    const [janeId, johnId] = represent(created.transactionId, ['Jane Smith', 'John Smith']);

    markPersonSatisfied(AGENT_ID, created.transactionId, janeId, 'reco_information_guide', {
      at: AT2, actor: 'agent', baseDir, now: LATER,
    });
    ...
    expect(row.satisfiedPersons).toEqual([janeId]);
    expect(row.outstandingPersons).toEqual([johnId]);
  });
  ...
  it('satisfying both persons empties outstandingPersons (proven through resolveTransactionChecklist)', () => {
    ...
    markPersonSatisfied(AGENT_ID, created.transactionId, johnId, 'reco_information_guide', {
      at: AT2, actor: 'agent', baseDir, now: LATER,
    });

    const resolved = resolveTransactionChecklist(AGENT_ID, created.transactionId, { baseDir });
    const row = resolved.find((item) => item.id === 'reco_information_guide');
    expect(row.satisfiedPersons).toEqual([janeId, johnId]);
  ...
  it('writing on an item that currently resolves indeterminate still succeeds, and the resolved row still carries neither field', () => {
    ...
    const result = markPersonSatisfied(AGENT_ID, created.transactionId, janeId, 'fintrac_corporation_identification_record', {
      at: AT2, actor: 'agent', baseDir, now: LATER,
    });

    expect(result.facts.clientSatisfactions[janeId]).toEqual({
  ...
  it('a satisfaction stored for a participant not represented is silently ignored by the resolver', () => {
    // Written directly via writeTransaction, bypassing markPersonSatisfied's
    // participant validation entirely, to pin the resolver's own behaviour:
    // the validation in this commit is the only thing that normally prevents
    // an orphaned entry like this from being written at all.
    const created = create();
    const [janeId] = represent(created.transactionId, ['Jane Smith']);
    const before = readTransaction(AGENT_ID, created.transactionId, { baseDir });
    ...
    writeTransaction(AGENT_ID, {
      ...
      facts: {
        ...before.facts,
        clientSatisfactions: {
          'per-00000000': { reco_information_guide: { at: AT, actor: 'agent' } },
        },
      },
    }, { baseDir, now: CLOCK });
    ...
    expect(row.outstandingPersons).toEqual([janeId]);
  });
});
...
describe('markPersonUnsatisfied', () => {
  it('writes a person_unsatisfied event with a payload of personId and itemId', () => {
    ...
    const result = markPersonUnsatisfied(AGENT_ID, created.transactionId, janeId, 'reco_information_guide', {
    ...
    expect(events[0].payload).toEqual({ personId: janeId, itemId: 'reco_information_guide' });
  });
  ...
  it('unsatisfying one item removes only that item key, leaving the person satisfied on their other items', () => {
    ...
    markPersonSatisfied(AGENT_ID, created.transactionId, janeId, 'fintrac_individual_identification_record', {
      at: AT2, actor: 'agent', baseDir, now: CLOCK,
    });
    ...
    expect(recoRow.outstandingPersons).toEqual([janeId]);
    expect(fintracRow.satisfiedPersons).toEqual([janeId]);
  ...
  it('unsatisfying the last item for a person removes the person key entirely, not just the item key', () => {
    ...
    expect(result.facts.clientSatisfactions).toEqual({});
    expect(Object.prototype.hasOwnProperty.call(result.facts.clientSatisfactions, janeId)).toBe(false);
  });
  ...
  it('throws when unsatisfying an item the person was never satisfied for, and writes nothing to disk', () => {
    ...
    expect(() => markPersonUnsatisfied(AGENT_ID, created.transactionId, janeId, 'reco_information_guide', {
      at: AT2, actor: 'agent', baseDir, now: LATER,
    })).toThrow(new RegExp(`^markPersonUnsatisfied: '${janeId}' is not satisfied for item 'reco_information_guide'`));
  ...
  it('throws when personId is not a participant on the transaction, and writes nothing to disk', () => {
    ...
    expect(() => markPersonUnsatisfied(AGENT_ID, created.transactionId, 'per-ffffffff', 'reco_information_guide', {
      at: AT2, actor: 'agent', baseDir, now: LATER,
    })).toThrow(/^markPersonUnsatisfied: 'per-ffffffff' is not a participant on transaction/);
  ...
  it('throws when the transaction does not exist', () => {
    expect(() => markPersonUnsatisfied(AGENT_ID, 'txn-20260715-00000000', 'per-00000000', 'reco_information_guide', {
      at: AT2, actor: 'agent', baseDir, now: LATER,
